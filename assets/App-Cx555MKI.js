import{i as e,n as t,t as n}from"./jsx-runtime-BnxRlLMJ.js";import{t as r}from"./Box-CFEfsCq7.js";import{a as i,i as a,r as o,t as s}from"./index-DVH1i26V.js";import{B as c,E as l,Hr as u,Ht as d,It as f,J as p,Jt as m,Kt as h,Qt as g,Rr as _,Xt as v,Zt as y,a as b,ar as x,d as S,f as C,ir as w,jt as T,ln as E,qt as D,r as O}from"./three.module-BrMVvVOI.js";import{t as k}from"./OrbitControls-CT6IiSB_.js";import{t as A}from"./lil-gui.module.min--1wMio4V.js";var j=e(t(),1),M=`// From three.js: position, uv, normal, time, etc.\r
\r
out vec2 vUv;\r
\r
void main() {\r
...
`,P=1024,F=class{constructor(e){this.baseScene=e,this.scene=new w,this.camera=new D(-1,1,1,-1,.1,10),this.camera.position.set(0,0,1),this.camera.lookAt(0,0,0);let t=new Float32Array(P*P*4);for(let e=0;e<P;e++)for(let n=0;n<P;n++){let r=e*P+n,i=Math.random()*Math.PI*2,a=.3+.7*Math.random();t[r*4+0]=a*Math.cos(i),t[r*4+1]=a*Math.sin(i),t[r*4+2]=Math.random()*.1-.05,t[r*4+3]=-.5}this.initialPositionsTexture=new l(t,P,P,E,c),this.initialPositionsTexture.minFilter=d,this.initialPositionsTexture.magFilter=d,this.initialPositionsTexture.needsUpdate=!0,this.shaderMaterial=new x({uniforms:{uPositionObjects:{value:this.baseScene.objects.map(e=>e.position)},uPosition0:{value:this.initialPositionsTexture},uPosition1:{value:this.initialPositionsTexture},uPosition2:{value:this.initialPositionsTexture},time:{value:0}},vertexShader:M,fragmentShader:N});let n=new T(new v(2,2),this.shaderMaterial);this.scene.add(n),this.fbos=[];for(let e=0;e<3;e++){let e=this.createRenderTarget();this.fbos.push(e),this.baseScene.renderer.setRenderTarget(e),this.baseScene.renderer.render(this.scene,this.camera)}this.currentFboIndex=0,this.baseScene.renderer.setRenderTarget(null)}createRenderTarget(){return new u(P,P,{minFilter:d,magFilter:d,format:E,type:c})}debugArray(){let e=new Float32Array(P*P*4);return this.baseScene.renderer.readRenderTargetPixels(this.fbos[0],0,0,P,P,e),[...e.filter((e,t)=>t%4==3)].sort(()=>Math.random()-.5).slice(0,20).sort((e,t)=>e-t)}step(e){let[t,n,r]=[this.currentFboIndex,(this.currentFboIndex+1)%3,(this.currentFboIndex+2)%3];this.shaderMaterial.uniforms.uPosition1.value=this.fbos[r].texture,this.shaderMaterial.uniforms.uPosition2.value=this.fbos[t].texture,e.setRenderTarget(this.fbos[n]),e.render(this.scene,this.camera),e.setRenderTarget(null),this.currentFboIndex=n}setObjectPositions(){for(let e=0;e<5;e++)this.shaderMaterial.uniforms.uPositionObjects.value[e]=this.baseScene.objects[e].position}dispose(){this.initialPositionsTexture.dispose();for(let e=0;e<3;e++)this.fbos[e].dispose();this.shaderMaterial.dispose()}},I=`uniform sampler2D particleMap;\r
...
`,R=class{constructor(e){this.cleanUpTasks=[],this.animationRequestID=null,this.lastTime=null,this.isStopped=!1,this.objects=[],this.objectRotationVectors=[],this.container=e,this.renderer=new O({antialias:!0,alpha:!0}),this.renderer.setClearColor(0,0),e.appendChild(this.renderer.domElement),h.DEFAULT_UP.set(0,1,0),this.scene=this.setupScene(),this.camera=this.setupCamera(),this.setupResizeRenderer(),this.resizeRenderer(),this.particleScene=new F(this),this.createGUI(),this.animate=this.animate.bind(this),this.animate()}resizeRenderer(){this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));let{clientWidth:e,clientHeight:t}=this.container;this.renderer.setSize(e,t),this.camera instanceof m&&(this.camera.aspect=e/t,this.camera.updateProjectionMatrix())}setupResizeRenderer(){let e=new ResizeObserver(()=>{this.resizeRenderer()});e.observe(this.container),this.cleanUpTasks.push(()=>e.unobserve(this.container))}createGUI(){this.gui=new A;let e={debugDialogButton:()=>alert(this.particleScene.debugArray()),animateButton:()=>this.animateStep(!1),toggleStop:()=>{this.isStopped=!this.isStopped}};this.gui.add(e,`debugDialogButton`).name(`Show alphas`),this.gui.add(e,`animateButton`).name(`Animate step`),this.gui.add(e,`toggleStop`).name(`Toggle stop/play`),this.gui.close()}dispose(){this.animationRequestID&&cancelAnimationFrame(this.animationRequestID);for(let e of this.cleanUpTasks)e();this.particleScene.dispose(),this.shaderMaterial.dispose(),this.renderer.dispose(),this.gui.destroy(),this.container.removeChild(this.renderer.domElement)}setupScene(){let e=new w,t=new p(.1,1),n=new f({flatShading:!0});for(let r=0;r<5;r++){let r=new T(t,n);this.objects.push(r),this.objectRotationVectors.push(new _().randomDirection()),e.add(r)}let r=new y(16777215,200,0);r.position.set(0,50,0),e.add(new b(14544639,.8)),e.add(r);let i=new C,a=new Float32Array(P*P*3);for(let e=0;e<P;e++)for(let t=0;t<P;t++){let n=e*P+t;a[n*3+0]=e/P,a[n*3+1]=t/P,a[n*3+2]=0}i.setAttribute(`position`,new S(a,3)),this.shaderMaterial=new x({uniforms:{particleMap:{value:null},time:{value:0}},vertexShader:I,fragmentShader:L});let o=new g(i,this.shaderMaterial);return o.frustumCulled=!1,e.add(o),this.moveObjects(0),e.rotateOnAxis(new _(1,0,0),-Math.PI/2),e}setupCamera(){let e=new m(60,1,.1,1e3),t=new k(e,this.container);return this.cleanUpTasks.push(()=>t.dispose()),e.position.set(0,1,1),e.lookAt(new _(0,0,0)),e}animate(){this.animationRequestID=requestAnimationFrame(this.animate),this.animateStep(this.isStopped)}animateStep(e){let t=(this.lastTime??0)+(e?0:.01);this.lastTime=t,this.particleScene.shaderMaterial.uniforms.time.value=t,e||(this.moveObjects(t),this.particleScene.setObjectPositions(),this.particleScene.step(this.renderer)),this.shaderMaterial.uniforms.particleMap.value=this.particleScene.fbos[this.particleScene.currentFboIndex].texture,this.renderer.render(this.scene,this.camera)}moveObjects(e){this.objects.forEach((t,n)=>{t.rotateOnAxis(this.objectRotationVectors[n],.2),t.position.set(1*Math.cos(n+.1*e),1*Math.sin(2*n+.2*e),.2)})}},z=n(),B=()=>{let e=(0,j.useRef)(null);return(0,j.useEffect)(()=>{console.log(`useEffect: `,e.current);let t=new R(e.current);return()=>{t.dispose()}},[]),(0,z.jsx)(`div`,{ref:e,style:{width:`100%`,height:`100%`}})},V=()=>(0,z.jsxs)(a,{maxWidth:`xl`,children:[(0,z.jsx)(r,{display:`flex`,justifyContent:`center`,sx:{py:2},children:(0,z.jsx)(i,{variant:`h2`,children:`Particles`})}),(0,z.jsx)(r,{style:{width:`100%`,height:`80vh`},children:(0,z.jsx)(B,{})}),(0,z.jsx)(o,{component:s,to:`/`,variant:`body1`,color:`primary`,children:`Back`})]});export{V as default};