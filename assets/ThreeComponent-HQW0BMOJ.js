import{i as e,n as t,t as n}from"./jsx-runtime-BnxRlLMJ.js";import{B as r,E as i,Hr as a,Ht as o,Jt as s,Kt as c,Qt as l,Rr as u,Xt as d,Zt as f,a as p,ar as m,d as h,f as g,ir as _,jt as v,ln as y,qt as b,r as x,vr as S}from"./three.module-BrMVvVOI.js";import{t as C}from"./OrbitControls-CT6IiSB_.js";import{t as w}from"./lil-gui.module.min--1wMio4V.js";var T=e(t(),1),E=`// From three.js: position, uv, normal, time, etc.\r
\r
varying vec2 vUv;\r
\r
void main() {\r
...
`,O=(e,t)=>{let n=20*Math.PI/180,r=62*Math.PI/180,i=e-n,a=t-r;return[5*i*Math.cos(r),5*a]},k=class{constructor(e){this.baseScene=e,this.scene=new _,this.camera=new b(-1,1,1,-1,.1,10),this.camera.position.set(0,0,1),this.camera.lookAt(0,0,0),this.computeInitialTextures(),this.shaderMaterial=new m({uniforms:{uPositionObjects:{value:Array.from({length:16},()=>new u(-100,0,0))},extraData:{value:this.initialExtraDataTexture},uPosition0:{value:this.initialPositionsTexture},uPosition1:{value:this.initialPositionsTexture},uPosition2:{value:this.initialPositionsTexture},time:{value:0}},vertexShader:E,fragmentShader:D});let t=new v(new d(2,2),this.shaderMaterial);this.scene.add(t),this.fbos=[];for(let e=0;e<3;e++){let e=this.createRenderTarget();this.fbos.push(e),this.baseScene.renderer.setRenderTarget(e),this.baseScene.renderer.render(this.scene,this.camera)}this.currentFboIndex=0,this.baseScene.renderer.setRenderTarget(null)}createRenderTarget(){return new a(128,128,{minFilter:o,magFilter:o,format:y,type:r})}debugArray(){let e=new Float32Array(16384*4);return this.baseScene.renderer.readRenderTargetPixels(this.fbos[0],0,0,128,128,e),[...e.filter((e,t)=>t%4==3)].sort(()=>Math.random()-.5).slice(0,20).sort((e,t)=>e-t)}step(e){let[t,n,r]=[this.currentFboIndex,(this.currentFboIndex+1)%3,(this.currentFboIndex+2)%3];this.shaderMaterial.uniforms.uPosition1.value=this.fbos[r].texture,this.shaderMaterial.uniforms.uPosition2.value=this.fbos[t].texture,e.setRenderTarget(this.fbos[n]),e.render(this.scene,this.camera),e.setRenderTarget(null),this.currentFboIndex=n}setObjectPositions(){for(let e=0;e<16;e++)this.shaderMaterial.uniforms.uPositionObjects.value[e]=new u(this.baseScene.appIconPositions[3*e+0],this.baseScene.appIconPositions[3*e+1],this.baseScene.appIconPositions[3*e+2])}computeInitialTextures(){let e=new Float32Array(16384*4),t=new Float32Array(16384*4);for(let n=0;n<128;n++)for(let r=0;r<128;r++){let i=n*128+r,a=Math.random()*Math.PI*2,o=.3+.7*Math.random();e[i*4+0]=1e3+o*Math.cos(a),e[i*4+1]=o*Math.sin(a),e[i*4+2]=0,e[i*4+3]=-.5,t[i*4+0]=0,t[i*4+1]=0,t[i*4+2]=0,t[i*4+3]=0}console.log(this.baseScene.data.scandinavia.pointsObj);let n=0,a=0;for(let r in this.baseScene.data.scandinavia.pointsObj){let i=this.baseScene.data.scandinavia.pointsObj[r];console.log(`country, points`,r,i);for(let r of i){let i=O(r[0]*Math.PI/180,r[1]*Math.PI/180),o=Math.floor(Math.random()*22);if(e[n*4+0]=i[0],e[n*4+1]=i[1],e[n*4+2]=0,t[n*4+0]=a,t[n*4+1]=o,t[n*4+2]=0,t[n*4+3]=0,n++,n>=16384)throw Error(`Particle texture size is too small.`)}a++}console.log(`index`,n,`${(100*n/16384).toFixed(1)}% of particles used`),this.initialPositionsTexture=new i(e,128,128,y,r),this.initialPositionsTexture.minFilter=o,this.initialPositionsTexture.magFilter=o,this.initialPositionsTexture.needsUpdate=!0,this.initialExtraDataTexture=new i(t,128,128,y,r),this.initialExtraDataTexture.minFilter=o,this.initialExtraDataTexture.magFilter=o,this.initialExtraDataTexture.needsUpdate=!0}},A=`uniform sampler2D particleMap;\r
