import{i as e,n as t,t as n}from"./jsx-runtime-BnxRlLMJ.js";import{B as r,Dt as i,G as a,K as o,Y as s,Z as c,b as l,c as u,lt as d,p as f,r as p,s as m,st as h,tt as g,ut as _,wt as v}from"./three.module-DeIRjVee.js";var y=e(t(),1),b=`varying vec2 vUv;\r
\r
void main() {\r
    vUv = uv;\r
...
}`,S=class{constructor(e){this.baseScene=e,this.scene=new d,this.camera=new o(-1,1,1,-1,.1,10),this.camera.position.set(0,0,1),this.camera.lookAt(0,0,0);let t=new Float32Array(512*512*4);for(let e=0;e<512;e++)for(let n=0;n<512;n++){let r=e*512+n,i=Math.random()*Math.PI*2,a=.5+.1*Math.random();t[r*4+0]=a*Math.cos(i),t[r*4+1]=a*Math.sin(i),t[r*4+2]=Math.random()*2*Math.PI,t[r*4+3]=.5}this.initialPositionsTexture=new f(t,512,512,g,l),this.initialPositionsTexture.minFilter=a,this.initialPositionsTexture.magFilter=a,this.initialPositionsTexture.needsUpdate=!0,this.shaderMaterial=new _({uniforms:{uPosition:{value:this.initialPositionsTexture},trailMap:{value:this.baseScene.fbos[0].texture},resolution:{value:this.baseScene.getResolution()},time:{value:0}},vertexShader:b,fragmentShader:x});let n=new r(new s(2,2),this.shaderMaterial);this.scene.add(n),this.fbos=[];for(let e=0;e<2;e++){let e=this.createRenderTarget();this.fbos.push(e),this.baseScene.renderer.setRenderTarget(e),this.baseScene.renderer.render(this.scene,this.camera)}this.currentFboIndex=0,this.baseScene.renderer.setRenderTarget(null)}createRenderTarget(){return new i(512,512,{minFilter:a,magFilter:a,wrapS:h,wrapT:h,format:g,type:l})}step(e){let[t,n]=[this.currentFboIndex,(this.currentFboIndex+1)%2];this.shaderMaterial.uniforms.uPosition.value=this.fbos[t].texture,e.setRenderTarget(this.fbos[n]),e.render(this.scene,this.camera),e.setRenderTarget(null),this.currentFboIndex=n}},C=`// From three.js: position, uv, normal\r
\r
