import{i as e,n as t,t as n}from"./jsx-runtime-BnxRlLMJ.js";import{Cr as r,Ft as i,Kt as a,Qt as o,U as s,Ur as c,Xt as l,Zt as u,a as d,en as f,k as p,lr as m,m as h,nn as g,p as _,pn as v,qr as y,r as b,tn as x,ur as S}from"./three.module-B6HkQTe3.js";import{t as C}from"./lil-gui.module.min-DjGmwgKf.js";import{t as w}from"./OrbitControls-Dh6wquD6.js";var T=e(t(),1),E=`// From three.js: position, uv, normal, time, etc.\r
\r
varying vec2 vUv;\r
\r
void main() {\r
...
`,O=(e,t)=>{let n=20*Math.PI/180,r=62*Math.PI/180,i=e-n,a=t-r;return[5*i*Math.cos(r),5*a]},k=class{constructor(e){this.baseScene=e,this.scene=new m,this.camera=new u(-1,1,1,-1,.1,10),this.camera.position.set(0,0,1),this.camera.lookAt(0,0,0),this.computeInitialTextures(),this.shaderMaterial=new S({uniforms:{uPositionObjects:{value:Array.from({length:16},()=>new c(-100,0,0))},extraData:{value:this.initialExtraDataTexture},uPosition0:{value:this.initialPositionsTexture},uPosition1:{value:this.initialPositionsTexture},uPosition2:{value:this.initialPositionsTexture},time:{value:0}},vertexShader:E,fragmentShader:D});let t=new i(new f(2,2),this.shaderMaterial);this.scene.add(t),this.fbos=[];for(let e=0;e<3;e++){let e=this.createRenderTarget();this.fbos.push(e),this.baseScene.renderer.setRenderTarget(e),this.baseScene.renderer.render(this.scene,this.camera)}this.currentFboIndex=0,this.baseScene.renderer.setRenderTarget(null)}createRenderTarget(){return new y(128,128,{minFilter:a,magFilter:a,format:v,type:s})}debugArray(){let e=new Float32Array(16384*4);return this.baseScene.renderer.readRenderTargetPixels(this.fbos[0],0,0,128,128,e),[...e.filter((e,t)=>t%4==3)].sort(()=>Math.random()-.5).slice(0,20).sort((e,t)=>e-t)}step(e){let[t,n,r]=[this.currentFboIndex,(this.currentFboIndex+1)%3,(this.currentFboIndex+2)%3];this.shaderMaterial.uniforms.uPosition1.value=this.fbos[r].texture,this.shaderMaterial.uniforms.uPosition2.value=this.fbos[t].texture,e.setRenderTarget(this.fbos[n]),e.render(this.scene,this.camera),e.setRenderTarget(null),this.currentFboIndex=n}setObjectPositions(){for(let e=0;e<16;e++)this.shaderMaterial.uniforms.uPositionObjects.value[e]=new c(this.baseScene.appIconPositions[3*e+0],this.baseScene.appIconPositions[3*e+1],this.baseScene.appIconPositions[3*e+2])}computeInitialTextures(){let e=new Float32Array(16384*4),t=new Float32Array(16384*4);for(let n=0;n<128;n++)for(let r=0;r<128;r++){let i=n*128+r,a=Math.random()*Math.PI*2,o=.3+.7*Math.random();e[i*4+0]=1e3+o*Math.cos(a),e[i*4+1]=o*Math.sin(a),e[i*4+2]=0,e[i*4+3]=-.5,t[i*4+0]=0,t[i*4+1]=0,t[i*4+2]=0,t[i*4+3]=0}console.log(this.baseScene.data.scandinavia.pointsObj);let n=0,r=0;for(let i in this.baseScene.data.scandinavia.pointsObj){let a=this.baseScene.data.scandinavia.pointsObj[i];console.log(`country, points`,i,a);for(let i of a){let a=O(i[0]*Math.PI/180,i[1]*Math.PI/180),o=Math.floor(Math.random()*22);if(e[n*4+0]=a[0],e[n*4+1]=a[1],e[n*4+2]=0,t[n*4+0]=r,t[n*4+1]=o,t[n*4+2]=0,t[n*4+3]=0,n++,n>=16384)throw Error(`Particle texture size is too small.`)}r++}console.log(`index`,n,`${(100*n/16384).toFixed(1)}% of particles used`),this.initialPositionsTexture=new p(e,128,128,v,s),this.initialPositionsTexture.minFilter=a,this.initialPositionsTexture.magFilter=a,this.initialPositionsTexture.needsUpdate=!0,this.initialExtraDataTexture=new p(t,128,128,v,s),this.initialExtraDataTexture.minFilter=a,this.initialExtraDataTexture.magFilter=a,this.initialExtraDataTexture.needsUpdate=!0}},A=`uniform sampler2D particleMap;\r
...
`,M=`uniform sampler2D particleMap;\r
...
`,P=class{constructor(e,t){this.animationRequestID=null,this.lastTime=null,this.isStopped=!1,this.appIconPositions=new Float32Array(48),this.container=e,this.data=t,this.cleanUpTasks=[],this.renderer=new b({antialias:!0,alpha:!0}),this.renderer.setClearColor(1519680,1),e.appendChild(this.renderer.domElement),l.DEFAULT_UP.set(0,1,0),this.scene=this.setupScene(),this.camera=this.setupCamera();for(let e=0;e<16;e++)this.initAppIcon(e,0);this.setupResizeRenderer(),this.resizeRenderer(),this.particleScene=new k(this),this.createGUI(),console.log(t.scandinavia),console.log(t.appAtlas),console.log(t.reactionAtlas),this.cleanUpTasks.push(()=>{this.animationRequestID&&cancelAnimationFrame(this.animationRequestID)}),this.animate=this.animate.bind(this),this.animate()}resizeRenderer(){this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));let{clientWidth:e,clientHeight:t}=this.container;this.renderer.setSize(e,t),this.camera instanceof o&&(this.camera.aspect=e/t,this.camera.updateProjectionMatrix());let n=Math.min(this.container.clientWidth/1200,1);this.camera.position.set(.3-1.3*n,1.7,0),this.camera.lookAt(new c(.3-1.3*n,0,0))}setupResizeRenderer(){let e=new ResizeObserver(()=>{this.resizeRenderer()});e.observe(this.container),this.cleanUpTasks.push(()=>e.unobserve(this.container))}createGUI(){this.gui=new C;let e={debugDialogButton:()=>alert(this.particleScene.debugArray()),animateButton:()=>this.animateStep(!1),toggleStop:()=>{this.isStopped=!this.isStopped}};this.gui.add(e,`debugDialogButton`).name(`Show alphas`),this.gui.add(e,`animateButton`).name(`Animate step`),this.gui.add(e,`toggleStop`).name(`Toggle stop/play`),this.gui.close()}cleanUp(){this.container.removeChild(this.renderer.domElement);for(let e of this.cleanUpTasks)e();this.renderer.dispose(),this.gui.destroy()}setupScene(){let e=new m,t=new x(16777215,200,0);t.position.set(0,50,0),e.add(new d(14544639,.8)),e.add(t);let n=new h,r=new Float32Array(16384*3);for(let e=0;e<128;e++)for(let t=0;t<128;t++){let n=e*128+t;r[n*3+0]=e/128,r[n*3+1]=t/128,r[n*3+2]=0}n.setAttribute(`position`,new _(r,3)),this.shaderReaction=new S({uniforms:{particleMap:{value:null},extraData:{value:null},time:{value:0},reactions:{value:this.data.reactionAtlas}},vertexShader:A,fragmentShader:j});let i=new g(n,this.shaderReaction);i.frustumCulled=!1,e.add(i);let a=new h;this.appIconPositionsAttribute=new _(this.appIconPositions,3),a.setAttribute(`position`,this.appIconPositionsAttribute),this.shaderApp=new S({uniforms:{particleMap:{value:null},time:{value:0},apps:{value:this.data.appAtlas}},vertexShader:M,fragmentShader:N});let o=new g(a,this.shaderApp);return o.frustumCulled=!1,e.add(o),this.moveAppIcons(0),e.rotateOnAxis(new c(1,0,0),-Math.PI/2),e}setupCamera(){let e=new o(60,1,.1,1e3),t=new w(e,this.container);return this.cleanUpTasks.push(()=>t.dispose()),e}animate(){this.animationRequestID=requestAnimationFrame(this.animate),this.animateStep(this.isStopped)}animateStep(e){let t=(this.lastTime??0)+(e?0:.002);this.lastTime=t,this.particleScene.shaderMaterial.uniforms.time.value=t,e||(this.moveAppIcons(t),this.particleScene.setObjectPositions(),this.particleScene.step(this.renderer)),this.shaderReaction.uniforms.particleMap.value=this.particleScene.fbos[this.particleScene.currentFboIndex].texture,this.shaderReaction.uniforms.extraData.value=this.particleScene.initialExtraDataTexture,this.renderer.render(this.scene,this.camera)}initAppIcon(e,t){let n=Math.min(this.container.clientWidth/1200,1);t>.4*e?(this.appIconPositions[3*e+0]=.24-2.2*n,this.appIconPositions[3*e+1]=-.5+.2*(Math.random()-.5),this.appIconPositions[3*e+2]=.2-.001*e):(this.appIconPositions[3*e+0]=-100,this.appIconPositions[3*e+1]=-100,this.appIconPositions[3*e+2]=.2-.001*e),this.appIconPositionsAttribute.needsUpdate=!0}moveAppIcons(e){let t=Math.min(this.container.clientWidth/1200,1);for(let n=0;n<16;n++){let r=t*1*.001*(2+Math.sin(7*e+n)+Math.sin(3*e+n)),i=t*2*(3.66-3*t)*5e-4*(1+.5*(Math.sin(n)+Math.sin(11*e+n)+Math.sin(4*e+n)));this.appIconPositions[3*n+0]=this.appIconPositions[3*n+0]+r,this.appIconPositions[3*n+1]=this.appIconPositions[3*n+1]+i,this.appIconPositions[3*n+2]=.2-.001*n,(Math.abs(this.appIconPositions[3*n+0])>2||Math.abs(this.appIconPositions[3*n+1])>1.5)&&this.initAppIcon(n,e)}this.appIconPositionsAttribute.needsUpdate=!0}},F=n(),I=()=>{let e=(0,T.useRef)(null),[t,n]=(0,T.useState)(),i=async()=>{console.log(`preload()`);try{let e=await Promise.all([fetch(`/dev-site-misc/geo/scandinavia.json`),fetch(`/dev-site-misc/socials/app_atlas.png`),fetch(`/dev-site-misc/socials/reaction_atlas.png`)]);for(let t of e)if(!t.ok)throw Error(`File fetch failed`);let t=await e[0].json(),i=await e[1].blob(),a=new r(await createImageBitmap(i));a.needsUpdate=!0;let o=await e[2].blob(),s=new r(await createImageBitmap(o));s.needsUpdate=!0,n({scandinavia:t,appAtlas:a,reactionAtlas:s})}catch(e){console.error(`Error loading JSON:`,e)}};return(0,T.useEffect)(()=>{if(console.log(`useEffect: `,e),!e.current)return;if(!t){i();return}let n=new P(e.current,t);return()=>{n.cleanUp()}},[t,e]),(0,F.jsx)(`div`,{style:{position:`fixed`,left:0,top:0,width:`100%`,height:`100%`,zIndex:`-1`,opacity:1},ref:e})};export{I as default};