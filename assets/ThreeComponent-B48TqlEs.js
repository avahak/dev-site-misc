var F=Object.defineProperty;var I=(r,e,t)=>e in r?F(r,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):r[e]=t;var n=(r,e,t)=>I(r,typeof e!="symbol"?e+"":e,t);import{r as d,j as T}from"./index-Q3QR4beh.js";import{l as b,p as R,q as y,r as f,s as p,N as m,t as x,V as P,u as E,f as w,v as O,W as j,m as g,I as M,w as U,n as Z,A as z,B as A,x as S,e as k,O as C}from"./OrbitControls-DV_4SOBH.js";const D=`// From three.js: position, uv, normal, time, etc.\r
\r
varying vec2 vUv;\r
...
vec3 computeForce(vec3 p0, vec3 p1, vec3 p2, float state) {\r
    ...
    vec3 vp = p2-uPositionObject;\r
\r
    float maxForce = 5.0;\r
    float maxSpeedHome = min(0.02, 0.1*length(v02));\r
    float maxSpeedOrbit = 0.1;\r
    // float maxForceOrbit = min(5., 5.*length(v)/maxSpeedOrbit+0.00001);\r
...
`;class H{constructor(e){n(this,"scene");n(this,"camera");n(this,"material");n(this,"SIZE",1024);n(this,"initialPositionsTexture");n(this,"fbos");n(this,"currentFboIndex");this.scene=new b,this.camera=new R(-1,1,1,-1,.1,10),this.camera.position.set(0,0,1),this.camera.lookAt(0,0,0);const t=new Float32Array(this.SIZE*this.SIZE*4);for(let s=0;s<this.SIZE;s++)for(let i=0;i<this.SIZE;i++){let o=s*this.SIZE+i,h=Math.random()*Math.PI*2,a=.5+.4*Math.random();t[o*4+0]=a*Math.cos(h),t[o*4+1]=a*Math.sin(h),t[o*4+2]=Math.random()*.1-.05,t[o*4+3]=1}this.initialPositionsTexture=new y(t,this.SIZE,this.SIZE,f,p),this.initialPositionsTexture.minFilter=m,this.initialPositionsTexture.magFilter=m,this.initialPositionsTexture.needsUpdate=!0,this.material=new x({uniforms:{uPositionObject:{value:new P(0,0,0)},uPosition0:{value:this.initialPositionsTexture},uPosition1:{value:this.initialPositionsTexture},uPosition2:{value:this.initialPositionsTexture},time:{value:0}},vertexShader:D,fragmentShader:_});const c=new E(2,2),l=new w(c,this.material);this.scene.add(l),this.fbos=[];for(let s=0;s<3;s++){const i=this.createRenderTarget();this.fbos.push(i),e.setRenderTarget(i),e.render(this.scene,this.camera)}this.currentFboIndex=2,e.setRenderTarget(null)}createRenderTarget(){return new O(this.SIZE,this.SIZE,{minFilter:m,magFilter:m,format:f,type:p})}step(e){const[t,c,l]=[this.currentFboIndex,(this.currentFboIndex+1)%3,(this.currentFboIndex+2)%3];this.material.uniforms.uPosition1.value=this.fbos[l].texture,this.material.uniforms.uPosition2.value=this.fbos[t].texture,e.setRenderTarget(this.fbos[c]),e.render(this.scene,this.camera),e.setRenderTarget(null),this.currentFboIndex=c}setObjectPosition(e){this.material.uniforms.uPositionObject.value=e}}const G=`// From three.js: position, uv, normal\r
\r
...
void main() {\r
    ...
    if (dist > 0.5)\r
        discard;\r
    vec4 color = vPosition.w < 0.5 ? vec4(0.2, 0.2, 1., 1.) : vec4(1., 0.2, 0.2, 1.);\r
    gl_FragColor = color;\r
}\r
`;class B{constructor(e){n(this,"container");n(this,"scene");n(this,"camera");n(this,"renderer");n(this,"cleanUpTasks");n(this,"animationRequestID",null);n(this,"lastTime",null);n(this,"cube",null);n(this,"fboScene");n(this,"material",null);this.container=e,this.cleanUpTasks=[],this.renderer=new j({antialias:!0,alpha:!0}),this.renderer.setClearColor(0,0),e.appendChild(this.renderer.domElement),this.fboScene=new H(this.renderer),this.scene=this.setupScene(),this.camera=this.setupCamera(),this.setupResizeRenderer(),this.resizeRenderer(),this.cleanUpTasks.push(()=>{this.animationRequestID&&cancelAnimationFrame(this.animationRequestID)}),this.animate=this.animate.bind(this),this.animate()}resizeRenderer(){this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));const{clientWidth:e,clientHeight:t}=this.container;this.renderer.setSize(e,t),this.camera instanceof g&&(this.camera.aspect=e/t,this.camera.updateProjectionMatrix())}setupResizeRenderer(){const e=new ResizeObserver(()=>{this.resizeRenderer()});e.observe(this.container),this.cleanUpTasks.push(()=>e.unobserve(this.container)),this.resizeRenderer()}cleanUp(){this.container.removeChild(this.renderer.domElement);for(const e of this.cleanUpTasks)e();this.renderer.dispose()}setupScene(){const e=new b,t=new M(.1,1),c=new U({flatShading:!0}),l=new w(t,c);e.add(l),this.cube=l;const s=new Z(16777215,200,0);s.position.set(0,50,0),e.add(new z(14544639,.8)),e.add(s);const i=new A,o=new Float32Array(this.fboScene.SIZE*this.fboScene.SIZE*2);for(let a=0;a<this.fboScene.SIZE;a++)for(let u=0;u<this.fboScene.SIZE;u++){let v=a*this.fboScene.SIZE+u;o[v*2+0]=a/this.fboScene.SIZE,o[v*2+1]=u/this.fboScene.SIZE}i.setAttribute("position",new S(new Float32Array(this.fboScene.SIZE*this.fboScene.SIZE*3),3)),i.setAttribute("uv",new S(o,2)),this.material=new x({uniforms:{uPosition:{value:null},time:{value:0}},vertexShader:G,fragmentShader:q});const h=new k(i,this.material);return h.frustumCulled=!1,e.add(h),e}setupCamera(){const e=new g(60,1,.1,1e3),t=new C(e,this.container);return this.cleanUpTasks.push(()=>t.dispose()),e.position.set(1,1,1.5),e.lookAt(new P(0,0,0)),e}animate(){this.animationRequestID=requestAnimationFrame(this.animate);const e=performance.now()/1e3,t=this.lastTime?Math.max(Math.min(e-this.lastTime,.1),0):0;this.lastTime=e,this.fboScene.material.uniforms.time.value=e,this.cube.rotateY(.1*t),this.cube.position.set(1*Math.cos(.1*e),1*Math.sin(.2*e),0),this.fboScene.setObjectPosition(this.cube.position),this.fboScene.step(this.renderer),this.material.uniforms.uPosition.value=this.fboScene.fbos[this.fboScene.currentFboIndex].texture,this.renderer.render(this.scene,this.camera)}}const W=()=>{const r=d.useRef(null);return d.useEffect(()=>{console.log("useEffect: ",r.current);const e=new B(r.current);return()=>{e.cleanUp()}},[]),T.jsx("div",{ref:r,style:{width:"100%",height:"100%"}})};export{W as default};
