...
This tree is a broad-phase spatial index for collision detection.

Let $S>1$ be a scaling factor.

The structure stores a set of objects represented by balls $O = B(c,r)$ with a fixed radius $r$ and a time-varying center $c$. The tree indexes them using a hierarchy of spherical regions. Every non-root node is a region $R = B(q, S^k)$, where $k \in \mathbb{Z}$ is the level of the region. The root is not geometric; it acts only as the top-level organizer. Regions of level $k_{\max}$ are the only children of the root, and there are no regions above level $k_{\max}$.

The algorithm operates in an abstract metric space, requiring no specific coordinate system. The underlying implementation must provide containment and intersection tests for balls, and a distance metric between centers for tie-breaking.

A fixed parameter $M > 1$ controls looseness. For an object $O = B(c,r)$, its admissible level is the unique integer $k$ such that

$$S^{k-1} \le Mr < S^k.$$

If $Mr \ge S^{k_{\max}}$, the object is too large for any region and is stored directly under the root.
...
* **Topology:** The parent of a level-$k$ region is either a level-$(k+1)$ region, or the root if $k = k_{\max}$.
* **Pruning:** Every region must contain at least one stored object or at least one child region.
* **Population:** A region is *populated* if it stores at least one object. Every populated region maintains a neighbor list containing exactly the populated regions that intersect it, including itself. Neighbor lists are symmetric: if region $A$ lists region $B$, then region $B$ lists region $A$. Every unpopulated region maintains an empty neighbor list.
...
An intersection query takes three inputs: a query ball $B$, a minimum level $k_{\min}$, and a boolean flag `group_by_level`.

The output is every region in the tree that intersects $B$ and whose level is at least $k_{\min}$. If `group_by_level = true`, the result is returned as a map from level to a list of matching regions.

The query is performed top-down, starting from the root's children. For each visited region, test for intersection with $B$. If the region intersects $B$ and its level is at least $k_{\min}$, include it in the output. Recurse only into children that intersect $B$. If a region's level is less than $k_{\min}$, it is not reported and its subtree is not traversed.

### Containment Query

A containment query takes the same inputs as an intersection query.

The output is every region in the tree that fully contains $B$ and whose level is at least $k_{\min}$. If `group_by_level = true`, the result is returned as a map from level to a list of matching regions.
...
2. If $Mr \ge S^{k_{\max}}$, store $O$ directly under the root and terminate.
...
6. **Connecting Upward:** Let $j := k$. Search the query results at level $j+1$ for a region that fully contains $R_j$.
   ...
   * If none exist and $j+1 \le k_{\max}$, create a new region $R_{j+1} = B(c,S^{j+1})$, make $R_j$ its child, set $j := j+1$, and repeat this step.
   * If $j = k_{\max}$ is reached without finding a parent, attach $R_{k_{\max}}$ directly to the root.
